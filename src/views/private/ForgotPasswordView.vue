<template>
  <section class="relative flex flex-wrap lg:h-screen lg:items-center">
    <div class="w-full px-4 py-12 lg:w-1/2 sm:px-6 lg:px-8 sm:py-16 lg:py-24">
      <div v-if="showMsg" class="max-w-lg mx-auto text-center">
        ...
        <h1 class="text-2xl font-bold sm:text-3xl">Email enviado com sucesso!</h1>
        <p class="mt-4 text-gray-500">
          Foi enviado ao seu email um link de redefinição de senha
        </p>
      </div>
      <div v-else>
        <div class="max-w-lg mx-auto text-center">
          <h1 class="text-2xl font-bold sm:text-3xl">Esqueci a minha senha</h1>
          <p class="mt-4 text-gray-500">
            Informe seu email e aguarde chegar o código de confirmação
          </p>
        </div>
        <form v-on:submit.prevent="forgotPassword" class="max-w-md mx-auto mt-8 mb-0 space-y-4">
          ...
          <div v-if=msg?.error>
            <span id="badge-dismiss-default"
              class="inline-flex items-center py-1 px-2 mr-2 text-sm font-medium text-white bg-red-600 rounded dark:bg-red-200 dark:text-red-800">
              {{ msg?.msg }}
            </span>
          </div>
...
import ForgotPassword from '../../composables/forgot-password'
export default defineComponent({
  name: 'ForgotPassword',
  setup() {
    const { sendMail, errors, showMsg, disabledClick, msg } = ForgotPassword()
